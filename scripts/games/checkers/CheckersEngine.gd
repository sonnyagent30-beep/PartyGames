...
func is_dark_square(r: int, c: int) -> bool: return (r + c) % 2 == 1
func piece_color(p: String) -> String:
	if p == "": return ""
	return p.to_lower()
func is_king(p: String) -> bool: return p != "" and p == p.to_upper()
func opponent(color: String) -> String: return "b" if color == "r" else "r"
...
func jumps_from(r: int, c: int, color: String) -> Array:
	...
	var results: Array = []
	for d in dirs:
		var mr: int = r + int(d[0]); var mc: int = c + int(d[1])
		var lr: int = r + 2*int(d[0]); var lc: int = c + 2*int(d[1])
	...
	if chained.is_empty(): return results
	return chained

func simple_moves_from(r: int, c: int, color: String) -> Array:
	if must_jump_from.size() > 0: return []
	var piece = piece_at(r, c)
	if piece == "" or piece_color(piece) != color: return []
	var dirs = [[-1,-1],[-1,1],[1,-1],[1,1]]
	if not is_king(piece):
	...
	for d in dirs:
		...
		if piece_at(nr, nc) == "": moves.append(Vector2i(nr, nc))
	return moves
...
func _refresh_must_jump() -> void:
	...
	for r in SIZE:
		for c in SIZE:
			...
			if not jumps_from(r, c, turn).is_empty(): must_jump_from.append(Vector2i(r, c))

func moves_from(r: int, c: int) -> Array:
	var piece = piece_at(r, c)
	if piece == "" or piece_color(piece) != turn: return []
	var jumps := jumps_from(r, c, turn)
	if not jumps.is_empty(): return jumps
	return []
	for m in simple_moves_from(r, c, turn): jumps.append([m])
	return jumps
...
func all_legal_paths(color: String) -> Array:
	...
	for r in SIZE:
		for c in SIZE:
			...
			for p in paths: result.append({"from": Vector2i(r, c), "path": p})
	if not result.is_empty(): return result
	for r in SIZE:
		for c in SIZE:
			if piece_color(piece_at(r, c)) != color: continue
			for m in simple_moves_from(r, c, color): result.append({"from": Vector2i(r, c), "path": [m]})
	return result
...
func apply_path(from: Vector2i, path: Array) -> bool:
	...
	for step in found_path:
		...
		if piece == piece.to_lower():
			var promoted := false
			if turn == "r" and current.x == SIZE-1: board[current.x][current.y] = "R"; promoted = true
			elif turn == "b" and current.x == 0: board[current.x][current.y] = "B"; promoted = true
	...
	var any_r := false; var any_b := false
	for r in SIZE:
		for c in SIZE:
			var p = piece_at(r, c)
			if p == "": continue
...
func serialize() -> Dictionary:
	var mj_arr = []
	for v in must_jump_from: mj_arr.append({"r": v.x, "c": v.y})
	return {"board": board.duplicate(true), "turn": turn, "game_over": game_over, "winner": winner, "history": move_history.duplicate(true), "must_jump_from": mj_arr}
...
func apply_snapshot(snap: Dictionary) -> void:
	...
	must_jump_from = []
	for entry in mj: must_jump_from.append(Vector2i(int(entry.get("r", -1)), int(entry.get("c", -1))))
	_refresh_must_jump()