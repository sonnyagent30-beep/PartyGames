...
##
## Per player we store:
##   ships: Array of {"name":String, "size":int, "cells":Array[Vector2i], "hits":int, "sunk":bool}
##   shots_fired: Dictionary key "r,c" -> {"hit":bool, "sunk_ship":String|null}
##   shots_received: same shape — what opponent has hit on us
##
## Both players' data lives on the host. Clients only ever receive their own
## view: their ships, their shots fired + results, opponent shots on them.
## Host ships never leak to client (and vice versa).
...
var turn: int = 0 # peer_id whose turn it is (only meaningful in battle)
var winner: int = 0 # peer_id or 0 for draw
...
func place_ship(peer_id: int, ship_name: String, cells: Array) -> bool:
	...
	# Validate ship exists and not already placed
	for s in p["ships"]:
		if s["name"] == ship_name:
			return false
	# Validate ship name
	...
	# Validate cells (size, contiguous, in bounds, not overlapping)
...
func auto_place(peer_id: int) -> bool:
	if phase != "placement": return false
	if not players.has(peer_id): return false
	# Clear existing ships for re-roll
	...
	for def in SHIP_FLEET:
		while attempts < 200:
			...
			for i in def["size"]:
				if horizontal:
					cells.append(Vector2i(r, c + i))
				else:
					cells.append(Vector2i(r + i, c))
			# Validate
			var ok := true
			for cell in cells:
				var v: Vector2i = cell
			...
			if overlap: continue
			if place_ship(peer_id, def["name"], cells):
				break
		else:
			return false
	return true
...
func fire(peer_id: int, r: int, c: int) -> Dictionary:
	"""Returns result dict with keys: ok, hit, sunk, ship_name, already, game_over, next_turn, winner."""
	...
	var key := "%d,%d" % [r, c]
	if players[peer_id]["shots_fired"].has(key):
		result["reason"] = "Already fired there"; return result
	if r < 0 or r >= SIZE or c < 0 or c >= SIZE:
		result["reason"] = "Out of bounds"; return result
